...
/*
 * Please keep the same order as in fpm_conf.h and in php-fpm2.conf.in
 */
...
static int fpm_conf_post_process(int force_daemon) /* {{{ */
{
	...
	if (!fpm_global_config.error_log) {
		fpm_global_config.error_log = strdup("log/php-fpm2.log");
	}
	...
	if (!fpm_global_config.syslog_ident) {
		fpm_global_config.syslog_ident = strdup("php-fpm2");
	}
...
int fpm_conf_init_main(int test_conf, int force_daemon) /* {{{ */
{
	...
	if (fpm_globals.config == NULL) {
		...
		if (fpm_globals.prefix == NULL) {
			spprintf(&tmp, 0, "%s/php-fpm2.conf", PHP_SYSCONFDIR);
		} else {
			spprintf(&tmp, 0, "%s/etc/php-fpm2.conf", fpm_globals.prefix);
		}
